...
void ld::linker::write_elf(const std::string& path) {
    ...
    uint32_t symtab_shidx = first_user + (uint32_t)user_sec_indices.size();
    ...
    uint32_t num_phdrs = (uint32_t)user_sec_indices.size();
    ...
    ehdr.e_entry = entry;
    ehdr.e_phoff = (Elf32_Off)ehdr_size;
    ehdr.e_shoff = (Elf32_Off)shoff;
    ehdr.e_flags = 0;
    ehdr.e_ehsize = (Elf32_Half)ehdr_size;
    ehdr.e_phentsize = (Elf32_Half)phdr_size;
    ...
    for (uint32_t i = 0; i < (uint32_t)user_sec_indices.size(); i++) {
        ...
        sh.sh_addr = (Elf32_Addr)m_section_table[si].address;
