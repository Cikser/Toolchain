...
void ld::linker::write_elf(const std::string& path) {
    ...
    // Za relocatable: za svaku sekciju koja ima relokacije, dodajemo .rela.<ime> sekciju
    // rela_sec_indices[i] = indeks u user_sec_indices sekcije čije su ovo relokacije
    std::vector<uint32_t> rela_sec_indices; // indeksi u user_sec_indices koji imaju relokacije
    if (is_rel) {
    ...
    for (auto& e : locals)  { push_sym(e, false); }
    uint32_t first_global_sym = (uint32_t)symtab_data.size();
    for (auto& e : globals) { push_sym(e, true);  }

    // Izgradnja Elf32_Rela vektora po sekciji
    // rela_data[i] odgovara rela_sec_indices[i]
    ...
    if (is_rel) {
        for (uint32_t i = 0; i < num_rela; i++) {
            uint32_t ui = rela_sec_indices[i];  // indeks u user_sec_indices
            ...
            sh.sh_info      = (Elf32_Word)(first_user + ui); // shdr indeks sekcije
            sh.sh_addralign = 4;
